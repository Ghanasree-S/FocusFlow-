...
// Updated to use 'gemini-3-pro-preview' for all operations including search grounding and image generation as requested
...
export const researchTopicForPrompt = async (
  ...
  // Fix: Standard generateContent call with search grounding tool
...
export const generateInfographicImage = async (prompt: string): Promise<string> => {
  ...
  // Fix: Removed config.responseModalities as it is not supported/needed for image generation with nano banana series models.
  const response = await getAi().models.generateContent({
    model: IMAGE_MODEL,
    contents: {
      parts: [{ text: prompt }]
  ...
  // Fix: Must iterate through parts to find the image part (inlineData) as it may not be the first part.
...
export const fixInfographicImage = async (currentImageBase64: string, correctionPrompt: string): Promise<string> => {
  ...
  // Fix: Removed responseModalities and implemented proper part iteration for image extraction.
  const response = await getAi().models.generateContent({
    model: EDIT_MODEL,
    contents: {
      parts: [
